...
public class WorkerRepository implements WorkerDAO {
	...
	public void add(Worker worker) {
		...
		jdbcTemplate.update(format, worker.getWorkerId(), worker.getFirstName(), worker.getLastName(),
				worker.getSalary(), worker.getJoiningDate(), worker.getDept(), worker.getEmail());
	...
	public void delete(int workerId) {
		String format = "DELETE FROM Worker where worker_id = ?";
	...
	public Worker getWorker(int workerId) {
		String format = "SELECT * FROM Worker WHERE Worker_id = ?";
